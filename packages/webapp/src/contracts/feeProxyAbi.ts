 ...
 *
 * Vendored verbatim from the canonical interface
 ...
 * inherits (roles, `paused()`, role events).
 *
 * Built with viem's `parseAbi` so the human-readable signatures stay in lockstep
 * with the Solidity interface. When Intuition publishes the compiled artifact,
 * swap the body for `import abi from './FeeProxy.json'; export const FeeProxyABI = abi`
