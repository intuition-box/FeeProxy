...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const updateMouse = (clientX: number, clientY: number): void => {
    ...
    const onMove = (ev: PointerEvent): void => updateMouse(ev.clientX, ev.clientY)
    const onLeave = (): void => {
      mouseTarget.set(0, 0)
    }
    ...
    const onCtxLost = (e: Event): void => {
      e.preventDefault()
      pausedRef.current = true
    }
    const onCtxRestored = (): void => {
    ...
    const adjustDprIfNeeded = (now: number): void => {
